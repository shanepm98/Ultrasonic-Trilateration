/*! \file post_main.c
 *
 * \brief Hardware bring-up entry point. Runs the ICU-20201 power-on self-test
 * (power_on_self_test.c) and reports the result to the USB serial console, retrying on
 * failure so the FFC / wiring can be re-seated and watched for recovery.
 *
 * This is a bring-up harness, not the production application - it only builds as part of the
 * src/hardware_bringup project.
 */
