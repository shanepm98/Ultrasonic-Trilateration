...
static const char *TAG = "bringup-tx";

static ch_group_t grp;
static ch_dev_t   dev;

void app_main(void) {
	ESP_LOGI(TAG, "ICU-20201 hardware bring-up: power-on self-test (pitch-catch sender)");
	...
	/* Hand off to the sender loop - configures the sensor for triggered TX/RX and idles,
	 * waiting to be fired by the receiver over the shared INT1 line. Does not return. */
	sender_run(&grp, &dev);
}
