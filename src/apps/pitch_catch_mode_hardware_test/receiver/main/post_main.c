 ...
 * src/apps/pitch_catch_mode_hardware_test/receiver app (pitch-catch fixed beacon; the sender has
 * its own app).
 */
...
static const char *TAG = "bringup-rx";

static ch_group_t grp;
static ch_dev_t   dev;

void app_main(void) {
	ESP_LOGI(TAG, "ICU-20201 hardware bring-up: power-on self-test (pitch-catch receiver)");
	...
	/* Hand off to the receiver loop - configures the sensor for triggered RX-only, then owns the
	 * periodic trigger loop that fires both sensors over the shared INT1 line. Does not return. */
	receiver_run(&grp, &dev);
}
