 ...
 *
 * This is the automated-tuning transmitter: it has no host connection of its own - it receives
 * its sensor configuration from the receiver over ESP-NOW (see at_protocol.h's
 * at_config_snapshot_t) and is passively fired over the shared INT1 trigger line.
 */
...
static const char *TAG = "at-tx";

static ch_group_t grp;
static ch_dev_t   dev;

void app_main(void) {
	ESP_LOGI(TAG, "ICU-20201 hardware bring-up: power-on self-test (automated-tuning transmitter)");
	...
	/* Hand off to the transmitter - brings up the ESP-NOW receive link and applies config
	 * snapshots as they arrive. Does not return. */
	at_transmitter_run(&grp, &dev);
}
